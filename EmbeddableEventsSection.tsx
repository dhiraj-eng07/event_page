...
// This can be replaced with actual data from your CMS or API
...
interface EmbeddableEventsSectionProps {
  events?: Event[];
  sectionId?: string;
  className?: string;
}

export function EmbeddableEventsSection({ 
  events = sampleEvents, 
  sectionId = "events",
  className = ""
}: EmbeddableEventsSectionProps) {
  ...
  return (
    <section 
      id={sectionId} 
      className={`min-h-screen bg-sky-100 relative ${className}`}
    >
      {/* Background overlay to ensure proper blending with existing site */}
      <div className="absolute inset-0 bg-gradient-to-b from-sky-50/50 to-sky-100"></div>
